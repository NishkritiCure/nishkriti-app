...
| Real `database.types.ts`          | phase-b | Regenerated from Supabase MCP in phase-a but the dev DB schema is still empty (0 tables). The type helpers are wired up correctly; `Tables`, `TablesInsert`, etc. will resolve to real types as soon as the backend team applies the schema from `APP-SCHEMA.md`. |
...
| Deviation                                                                         | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Entry files live in `src/entry/` instead of `src/app/` (per `ARCHITECTURE.md` §2) | Expo SDK 54's CLI treats `src/app/` as the auto-scanned Expo Router routes directory even when `expo-router` is not a dependency. Putting our non-router entry files there crashes Metro with `TypeError: path must be string` because `providers.tsx` / `RootNavigator.tsx` are not valid route exports. The rename preserves the spec's intent (entry shell composing providers + root navigator) without triggering the Expo Router collision. `registerRootComponent` lives at repo root `index.ts` (the canonical Expo non-router pattern), and `src/entry/index.tsx` just exports the `App` component. |
